...
import DSTV from "@/public/assets/bill/dstv.png";
import GOTV from "@/public/assets/bill/gotv.png";
import STARTIME from "@/public/assets/bill/startime.jpeg";
import ShowMax from "@/public/assets/bill/showmax.png";
...
const TvPage = () => {
  ...
  const { setShow, setShowVariatiom } = useBillStoreOverlay();
  ...
  const handleGetVariation = async () => {
    if (paymentDetails.value === undefined) return;
    try {
      const toastId = toast.loading("Fetching plan details...");
      // setIsLoading(true);

      const response = await getVariation(paymentDetails.value);
      // console.log(response);

      if (response.response_description === "000") {
        setPlans(response.content.varations);
      } else {
  ...
  return (
    <div className="w-full mt-4">
          ...
          <div className="w-full max-w-[640px] mx-auto   rounded-tl-[42px] rounded-tr-[42px] flex flex-col gap-[42px] justify-center items-center">
            <div className="flex w-full flex-col gap-4 justify-center items-center">
              <div className="w-full text-center">
                ...
                <span className="text-[14px] font-normal font-inter text-center  text-[#464646] leading-[28px] mt-2 inline-block">
                  Renew your cable subscription plan.
                </span>
              </div>
            </div>
            ...
            <div className="w-full bg-white rounded-tl-[42px] rounded-tr-[42px] lg:px-[72px] lg:pt-16 gap-6 p-[30px]">
              ...
              >
                ...
                <div>
                  ...
                  <Controller
                    ...
                    render={({ field: { onChange, value } }) => (
                      <button
                        onClick={() => {
                          setShowVariatiom && setShowVariatiom(true);

                          if (paymentDetails?.variation_name) {
                            onChange(paymentDetails.variation_name);
                          }
                        }}
                        type="button"
                        className="h-[60px] w-full mt-4 py-[15px] px-[16px] rounded-[10px] border border-[#faf7ff] outline-0 bg-[#FAF7FF] text-[#B4ACCA] text-base font-normal font-inter flex items-center justify-between"
                      >
                        {paymentDetails.variation_name
                          ? paymentDetails.variation_name
                          : "Service provider"}
                ...
                <Label htmlFor="meter_number" className="flex flex-col gap-4">
                  ...
                  <Input
                    ...
                    className="h-[60px] mt-2 py-[15px] px-[16px] rounded-[10px] border border-[#faf7ff] outline-0 bg-[#FAF7FF] placeholder:text-base placeholder:font-normal placeholder:text-placeholder-100 focus-visible:ring-[#faf7ff] focus-visible:ring-offset-0 placeholder:font-inter font-inter text-base font-normal"
                    maxLength={13}
                  ...
                  <div className="flex items-center gap-2 rounded-[10px] bg-[#FAF7FF] mt-2">
                    ...
                    <Input
                      {...register("amount", {
                        ...
                        value: paymentDetails.variation_amount,
                      })}
                      readOnly
                      disabled={isLoadings}
                      name="amount"
      ...
      <VariationModal paymentList={plans} />
    </div>
  );
};
