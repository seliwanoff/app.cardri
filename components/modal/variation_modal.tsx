...
const VariationModal = ({ paymentList }: PaymentModalProps) => {
  const { showVariation, setShowVariatiom, setPaymentDetails, paymentDetails } =
    useBillStoreOverlay();
  ...
  return (
    <Transition show={showVariation}>
      {/*** @ts-ignore */}
      <Dialog className="relative z-10" onClose={setShowVariatiom}>
        <TransitionChild
              ...
              <DialogPanel className="relative transform w-full lg:rounded-[30px] bg-[#F5F2FB] px-4 pb-4 pt-5 text-left lg:pl-[30px] lg:pb-[30px] lg:pr-[30px] shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-[526px] sm:p-6">
                <div className="flex w-ful items-center justify-between gap-4">
                  ...
                  <div
                    ...
                    onClick={() => setShowVariatiom && setShowVariatiom(false)}
                  >
                    <XIcon color="#6C757D" />
                  </div>
                </div>
                  ...
                  {paymentList?.map((item: any, index: any) => (
                    ...
                    >
                      <div className="flex   flex-col gap-1">
                        ...
                        <span className="text-sm font-inter  text-[#333] ">
                          {currencySymbols("NGN")}
